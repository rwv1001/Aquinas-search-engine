...
function updateLatinDisplay(value) {
    const latinCells = document.querySelectorAll(".search-result-text table tr, .group-search-result-text table tr");

    latinCells.forEach((row) => {
        // Count how many cells (td or th) are in this row
        const cells = row.querySelectorAll("td, th");
        const hasSecondColumn = cells.length > 1;
        const firstCell = cells[0];

        if (firstCell && hasSecondColumn) {
            if (value === "hide") {
...
window.updateLatinDisplay = updateLatinDisplay;

function UpdateLatinDisplayFromSaveSetting() {
    // Restore setting from localStorage if needed
    const savedSetting = localStorage.getItem("latinDisplay");
    if (savedSetting) {
        const savedRadio = document.querySelector(`input[name="latin-display"][value="${savedSetting}"]`);
        if (savedRadio) {
            savedRadio.checked = true;
            updateLatinDisplay(savedSetting);
        }
    }
}
window.UpdateLatinDisplayFromSaveSetting = UpdateLatinDisplayFromSaveSetting;
function LatinDisplayInitializer() {

    const radios = document.querySelectorAll('input[name="latin-display"]');

  

  // Initial setting based on default checked radio


  // Listen for changes
  radios.forEach((radio) => {
    radio.addEventListener("change", function () {
      updateLatinDisplay(this.value);
      // Optionally persist in localStorage
      localStorage.setItem("latinDisplay", this.value);
    });
  });
}
window.LatinDisplayInitializer = LatinDisplayInitializer;

function LoginStatusInitializer() {
window.addEventListener("refresh:loginStatuss", () => {
    fetch("/users/login_status", {
      headers: { "Accept": "text/html" }
    })
    .then(r => r.text())
    .then(html => {
      document.getElementById("login-status-div-id").innerHTML = html;
    });
  });
}
window.LoginStatusInitializer = LoginStatusInitializer;
